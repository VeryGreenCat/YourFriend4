...
class BotCreateModal(discord.ui.Modal, title="สร้าง Bot ของคุณ"):
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        embed = discord.Embed(
            title="✅ Bot สร้างเรียบร้อย!", color=discord.Color.green()
        )
        ...
        if failures:
            ...
            print(f"[bot_create] Extraction returned empty: {failures}. See logs/extractor_failures.log for details.")

        embed.add_field(name="Name", value=name, inline=True)
        embed.add_field(name="Traits", value=f"{len(traits)} traits analyzed", inline=True)
        embed.add_field(name="World Views", value=f"{len(world_views)} extracted", inline=True)
        embed.add_field(name="Emotion Conditions", value=f"{len(conditions)} detected", inline=True)
